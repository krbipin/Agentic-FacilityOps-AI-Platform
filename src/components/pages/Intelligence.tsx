...
export function Intelligence() {
  ...
  const showCorrelations = filter === "All" || filter === "Correlations";
  const showAnomalies = filter === "All" || filter === "Anomalies";
  const showForecasts = filter === "All" || filter === "Forecasts";
  const showAll = filter === "All";
  const healthKey = (agent: string) => agent.charAt(0).toUpperCase() + agent.slice(1);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Cost Reduction" value={fmtPct(kpis.cost_reduction_pct)} icon="dollar" delta={`${kpis.cost_reduction_pct >= 0 ? "▼" : "▲"} vs last month`} deltaTone={kpis.cost_reduction_pct >= 0 ? "green" : "red"} sub="Compared to previous month" />
      ...
      <div className={`mt-6 grid grid-cols-1 gap-6 ${showHealth && showCorrelations ? "xl:grid-cols-2" : "xl:grid-cols-1"}`}>
        {showHealth && (
          <Card className={filter === "Health" ? "ring-1 ring-violet/30" : ""}>
            <CardHeader title="Facility Health Score" subtitle="Per-agent health sub-scores" />
              ...
              <div className="min-w-[200px] flex-1 space-y-2.5">
                {Object.entries(agent_health).map(([agent, score]) => (
                  <div key={agent} className="flex items-center gap-3 text-body-sm">
                    <span className="w-28 shrink-0 truncate text-steel-slate capitalize">{agent}</span>
                    <div className="relative h-2.5 flex-1 overflow-hidden rounded-full bg-elevated-slate" role="img" aria-label={`${agent} ${score}`}>
                      <div className="h-full rounded-full" style={{ width: `${score}%`, background: agentColor[healthKey(agent)] ?? "var(--color-violet)" }} />
                    </div>
                    <span className="w-8 shrink-0 text-right font-mono text-caption text-ice-white">{score}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <Card className={filter === "Correlations" ? "ring-1 ring-violet/30" : ""}>
            ...
            {correlations.length === 0 ? (
              <div className="flex flex-col items-center gap-2 py-10 text-center">
                <Icon name="activity" className="text-steel-slate" size={28} />
                <p className="text-body-sm text-ice-white">No correlations detected yet</p>
                <p className="text-caption text-steel-slate">Cross-agent patterns appear once enough live data is collected.</p>
              </div>
            ) : (
                  ...
                  <div key={c.pair} className="rounded-control border border-hairline-slate bg-elevated-slate p-3.5">
                    ...
                    >
                      <span className="min-w-0">
                        <span className="block text-body-sm font-medium text-ice-white">{c.pair}</span>
                        <span className="mt-0.5 block text-caption text-steel-slate">confidence {c.confidence ?? "—"}</span>
                      </span>
                      <span className="shrink-0 font-mono text-body-md text-violet">
                        {typeof c.r === "number" ? `r=${Math.abs(c.r) < 0.005 ? "0.00" : c.r.toFixed(2)}` : "—"}
                      </span>
                    </button>
                    {expandedCorr === c.pair && (
      ...
      <div className={`mt-6 grid grid-cols-1 gap-6 ${showAnomalies && showForecasts ? "xl:grid-cols-2" : "xl:grid-cols-1"}`}>
        {showAnomalies && (
          <Card className={filter === "Anomalies" ? "ring-1 ring-violet/30" : ""}>
            <CardHeader title="Anomaly Detection Feed" subtitle="Across all agents · today" />
            <ul role="list" className="divide-y divide-hairline-slate">
              {anomaly_feed.length === 0 && (
                <li className="flex flex-col items-center gap-2 py-10 text-center">
                  <Icon name="check" className="text-signal-green" size={28} />
                  <p className="text-body-sm text-ice-white">No anomalies — all systems nominal</p>
                  <p className="text-caption text-steel-slate">The Intelligence Engine will flag unusual patterns here when it detects them.</p>
                </li>
              )}
              {anomaly_feed.map((a) => (
                <li key={`${a.domain}-${a.timestamp}`} className="flex flex-wrap items-center gap-x-2 gap-y-1.5 py-3">
                  <Chip tone={sevTone[a.severity] ?? "amber"}>{a.severity}</Chip>
                  <Chip tone="steel">{a.domain}</Chip>
                  <span className="min-w-0 flex-1 basis-56 truncate text-body-sm text-ice-white">{a.detail}</span>
        ...
        {showForecasts && (
          <Card className={filter === "Forecasts" ? "ring-1 ring-violet/30" : ""}>
            ...
            {forecasts.length === 0 ? (
              <div className="flex flex-col items-center gap-2 py-10 text-center">
                <Icon name="trendingUp" className="text-steel-slate" size={28} />
                <p className="text-body-sm text-ice-white">No forecasts available</p>
                <p className="text-caption text-steel-slate">Forecasts will generate once enough historical data is recorded.</p>
              </div>
            ) : (
                  ...
                  <div key={f.domain} className="rounded-control border border-hairline-slate bg-elevated-slate p-3.5">
                    ...
                    <div className="pt-2">
                      <AreaChart
                        data={f.series}
                        height={70}
                        color={agentColor[f.domain] ?? "var(--color-violet)"}
                        fill={agentColor[f.domain] ?? "var(--color-violet)"}
                        showGrid={false}
                        labelEvery={Math.max(1, Math.ceil(f.series.length / 6))}
                      />
                    </div>
                  </div>
                ))}
              ...
              <table className="w-full min-w-[640px] text-left text-body-sm">
                <thead>
                  <tr className="text-caption uppercase tracking-wide text-steel-slate">
                    ...
                    <th scope="col" className="py-2 pr-4 font-medium">Impact</th>
                ...
                <tbody className="divide-y divide-hairline-slate">
                  {recommendations.length === 0 && (
                    <tr>
                      <td colSpan={5} className="py-8 text-center text-body-sm text-steel-slate">No recommendations yet</td>
                    </tr>
                  )}
                  {recommendations.slice(0, 5).map((r, i) => (
                    <tr key={r.id ?? r.title} className="text-steel-slate">
                      ...
                      <td className="max-w-[320px] truncate py-3 pr-4 text-ice-white">{r.title}</td>
          ...
          <Card className="mt-6">
            ...
            {collaboration.length === 0 ? (
              <div className="flex flex-col items-center gap-2 py-10 text-center">
                ...
                <p className="text-body-sm text-ice-white">No agent interactions yet</p>
                <p className="text-caption text-steel-slate">The collaboration map updates as agents exchange insights.</p>
              </div>
            ) : (
              <ul role="list" className="space-y-2.5">
                {collaboration.map((e) => (
                  <li key={`${e.source}-${e.target}`} className="flex flex-wrap items-center gap-2 rounded-control border border-hairline-slate bg-elevated-slate px-4 py-3">
                    <span className="inline-flex shrink-0 items-center gap-1.5 text-body-sm text-ice-white">
                    ...
                    <Icon name="chevronRight" size={14} className="shrink-0 text-steel-slate" />
                    <span className="min-w-0 flex-1 basis-56 truncate text-body-sm text-steel-slate">{e.insight}</span>
                    <Icon name="chevronRight" size={14} className="shrink-0 text-steel-slate" />
                    <span className="inline-flex shrink-0 items-center gap-1.5 text-body-sm text-ice-white">
